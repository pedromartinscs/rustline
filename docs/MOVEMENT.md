...
MovementLab collision is protected by the release-critical `TilemapCompositeColliderInitializer2D`. Windows Player builds have repeatedly started with empty/unready Composite geometry even while Editor Play Mode looked correct. The accepted startup sequence is `TilemapCollider2D.ProcessTilemapChanges() → CompositeCollider2D.GenerateGeometry() → Physics2D.SyncTransforms()`, executed in `Awake`, repeated in `Start`, and repeated during the first two `FixedUpdate` ticks. The later passes are intentional: an Awake-only repair proved timing-sensitive as MovementLab grew. The hidden collision Tilemap must remain `Ground` layer 6 with an enabled `TilemapCollider2D` using `Merge`, an enabled polygon `CompositeCollider2D`, a static `Rigidbody2D`, and the initializer component. See [`RELEASE_COLLISION.md`](RELEASE_COLLISION.md) before changing this pipeline.
