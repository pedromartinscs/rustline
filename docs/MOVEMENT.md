...
The Body and Unarmed Arms sheets contain five `48×64` frames keyed at `0.00`, `0.10`, `0.20`, `0.30`, and `0.40` seconds. At 16 PPU, the physical Rigidbody2D root snaps through right-side authored offsets `(0,0)`, `(0.5,0.5)`, `(0.875,0.875)`, `(1.125,1.0625)`, and `(1.1875,1.1875)`; left-side traversal negates only X. After frame 4 is reached, that frame holds while a deterministic `0.16` second cubic ease settles to the validated destination. The collider is disabled only during committed corner crossing, then restored to the normal standing geometry. Completion zeros velocity, sets grounded state, and returns directly to Idle/Run without emitting `Landed`, so there is no Land presentation or root teleport.
...
| Ledge authored frame / final settle | 0.10 s / 0.16 s |
