...
The Body and Unarmed Arms sheets contain five `48×64` frames keyed at `0.00`, `0.10`, `0.20`, `0.30`, and `0.40` seconds. At 16 PPU, the physical Rigidbody2D root snaps through right-side authored offsets `(0,0)`, `(0.5,0.5)`, `(0.875,0.875)`, `(1.125,1.0625)`, and `(1.1875,1.1875)`; left-side traversal negates only X. Frame 4 remains at that calibrated ledge-contact position for its full final `0.10` second interval and is never translated upward after the hands reach the edge. At the exact `0.50` second state boundary, presentation leaves LedgeClimb while the physics root is atomically placed at the already-validated standing destination and the collider is restored to normal standing geometry. Completion zeros velocity, sets grounded state, and returns directly to Idle/Run without emitting `Landed`.

Once started, movement release, opposite movement, aim changes, Jump, Crouch, and Fire cannot cancel or mirror the climb. Jump takeoff presentation is reset to the normal Visual baseline on entry. Longwatch has no LedgeClimb carry artwork: it releases `ArmsWeaponSpriteRenderer` to the five-frame unarmed presenter, and firing is blocked for the full traversal.
