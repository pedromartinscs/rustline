...
Both logical textures are persistent and are recreated only when required logical dimensions change. They use ARGB32 sRGB color, Point filtering, clamp wrapping, no mipmaps, no anisotropy, no MSAA, and no HDR. The resolved/penumbra target is now depthless because it is only a RenderGraph color attachment and is never a camera output. At the maximum logical viewport this removes a requested 16-bit attachment of 2,298,368 bytes (about 2.19 MiB), subject to platform allocation details. The World Camera target conservatively retains its 16-bit depth request until the depthless camera-output path can be visually verified in Unity 6000.4; no theoretical saving is accepted at the expense of 2D rendering certainty. `Renderer2D.asset` remains `m_UseDepthStencilBuffer: 0`.

`_LogicalSize` is sent only when logical dimensions/material lifetime require it. Player pixel center and world-pixel Bayer origin are exact-dirty-checked, and stationary player/camera/projection inputs bypass `WorldToViewportPoint` plus material vector writes. RenderGraph source texture/scale-bias binding remains inside the execution callbacks because it is part of correct imported-texture sampling and orientation.
